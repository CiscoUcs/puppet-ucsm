...
Puppet::Type.type(:ucsm_san_connectivity).provide :ruby do
  ...
  def handle
     ...
     param_obj[:wwnn_pool]=@resource[:wwnn_pool]
     param_obj[:vhba_list]=@resource[:vhba_list]
     ...
     current = Puppet::Util::Execution.execute(
      "python #{path}/san_connectivity.py #{json_object}",
      :failonfail => true
    )
