...
Puppet::Type.type(:ucsm_disk_group_policy).provide :ruby do
  ...
  def handle
     ...
     param_obj[:raid_level]=@resource[:raid_level]
     param_obj[:slot_numbers] = @resource[:slot_numbers]
     ...
     current = Puppet::Util::Execution.execute(
      "python #{path}/disk_group_policy.py #{json_object}",
      :failonfail => true
    )
...
def green(text); colorize(text, 32); end
  ...
  def exists?
	...
	path = File.join(File.dirname(__FILE__), '..', '..', '..')
     	current = Puppet::Util::Execution.execute(
      	"python #{path}/query_disk_group_config_mo.py #{json_object}",
      	:failonfail => true
    	)
 	if(current.eql? "true")
	return true
  ...
  def self.get_instance(resource)
	...
	json_object=JSON.dump param_obj.to_json
        path = File.join(File.dirname(__FILE__), '..', '..', '..')
        current = Puppet::Util::Execution.execute(
        "python #{path}/diskGroupPolicyInstances.py #{json_object}",
        :failonfail => true
        )
  end
