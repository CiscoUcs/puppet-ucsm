...
Puppet::Type.type(:ucsm_vhba_template).provide :ruby do
  ...
  def handle
     ...
     param_obj[:fabric] = @resource[:fabric]
     param_obj[:redundancy_type]=@resource[:redundancy_type]
     param_obj[:vsan]=@resource[:vsan]
     param_obj[:template_type]=@resource[:template_type]
     param_obj[:max_data]=@resource[:max_data]
     param_obj[:wwpn_pool]=@resource[:wwpn_pool]
     param_obj[:qos_policy]=@resource[:qos_policy]
     param_obj[:pin_group]=@resource[:pin_group]
     param_obj[:stats_policy]=@resource[:stats_policy]
     param_obj[:org_dn]=@resource[:org_dn]
     param_obj[:check_exists]=false
     ...
     current = Puppet::Util::Execution.execute(
      "python #{path}/vhba_template.py #{json_object}",
      :failonfail => true
    )
