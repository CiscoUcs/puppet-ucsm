...
Puppet::Type.type(:ucsm_serverpool).provide :ruby do
  ...
  def handle
     ...
     param_obj[:pooled_servers] = @resource[:pooled_servers]
     ...
     current = Puppet::Util::Execution.execute(
      "python #{path}/server_pool.py #{json_object}",
      :failonfail => true
    )
...
def green(text); colorize(text, 32); end
  ...
  def exists?
	...
	path = File.join(File.dirname(__FILE__), '..', '..', '..')
     	current = Puppet::Util::Execution.execute(
      	"python #{path}/query_server_pool_mo.py #{json_object}",
      	:failonfail => true
    	)
 	if(current.eql? "true")
	return true
  ...
  def self.get_instance(resource)
	...
	json_object=JSON.dump param_obj.to_json
        path = File.join(File.dirname(__FILE__), '..', '..', '..')
        current = Puppet::Util::Execution.execute(
        "python #{path}/serverpoolInstances.py #{json_object}",
        :failonfail => true
        )
  end
