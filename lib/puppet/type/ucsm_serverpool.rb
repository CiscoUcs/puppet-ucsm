Puppet::Type.newtype(:ucsm_serverpool) do
  desc "Puppet type that manages serverpool object"
  ensurable
  ...
  newproperty(:descr) do
    desc "The description of the managed object(This is with respect to ucspe)"
        validate do |value|
          if value.length > 16
            raise  ArgumentError, "The description exceeds maximum character length of 16"
          end
        end

  end

  newproperty(:pooled_servers, :array_matching => :all) do
    desc "pooled_servers contains list of dictionary objects"
  end
